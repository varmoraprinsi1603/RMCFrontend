...
function Reports() {
  ...
  const [performanceExcelLoading, setPerformanceExcelLoading] =
    useState(false);
  ...
  // =====================================================
  // GET TICKET PERFORMANCE
  // =====================================================

  const getTicketPerformance = async () => {
    try {
      ...
      if (performanceFilters.FromDate) {
        params.append(
          "FromDate",
          performanceFilters.FromDate
        );
      }

      if (performanceFilters.ToDate) {
        params.append(
          "ToDate",
          performanceFilters.ToDate
        );
      }

      if (performanceFilters.Status) {
        params.append(
          "Status",
      ...
      if (performanceFilters.Priority) {
        params.append(
          "Priority",
          performanceFilters.Priority
        );
      }

      if (
        performanceFilters.AssignedToName.trim()
      ) {
        params.append(
          "AssignedToName",
          performanceFilters.AssignedToName.trim()
      ...
      const response = await fetch(
        `${API_BASE}/GetTicketPerformanceReport?${params.toString()}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        throw new Error(
          result?.Message ||
            result?.message ||
            "Unable to load ticket performance."
        );
      }
  ...
  // =====================================================
  // PERFORMANCE EXCEL DOWNLOAD
  // =====================================================

  const handlePerformanceExcelDownload =
    async () => {
      try {
        setPerformanceExcelLoading(true);
        setMessage("");
        ...
        const response = await fetch(
          `${API_BASE}/ExportTicketPerformanceExcel?${params.toString()}`,
          {
            method: "GET",
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        setPerformanceExcelLoading(false);
      }
    };
    ...
    <div>
      ...
      <div className="bg-white border border-[#dce6ef] rounded-2xl shadow-sm">
        ...
        <div className="px-5 py-3 border-t border-[#e7edf3] flex justify-end">
          ...
          <button
            type="button"
            onClick={getTicketPerformance}
            disabled={performanceLoading}
            className="h-9 px-5 rounded-lg bg-[#40566b] text-white text-[12px] font-semibold hover:bg-[#344b60] transition disabled:opacity-60 flex items-center gap-2"
          >
        ...
        <div className="mt-5 bg-white border border-[#dce6ef] rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="px-5 py-3 border-b border-[#e7edf3] flex items-center justify-between">
            ...
            <div>
              ...
              <div className="text-[10px] text-[#8093a7] mt-0.5">
                Support executive performance summary
              </div>
            ...
            <button
              type="button"
              onClick={
                handlePerformanceExcelDownload
              }
              disabled={
                performanceExcelLoading
              }
              className="h-9 px-4 rounded-lg bg-[#3f6f55] text-white text-[11px] font-semibold flex items-center gap-2 hover:bg-[#345d47] transition disabled:opacity-60"
            >
            ...
            <table className="w-full text-left">
              ...
              <thead>
                ...
                <tr className="bg-[#40566b] text-white">
                  ...
                  <th className="px-4 py-3 text-[11px] font-semibold whitespace-nowrap text-center">
                    Total Assigned
                  </th>
                ...
                {performanceData.map(
                  (item, index) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-[11px] text-[#526b83] text-center whitespace-nowrap">
                        {item.TotalAssigned ??
                          item.totalAssigned ??
                          0}
                      </td>
